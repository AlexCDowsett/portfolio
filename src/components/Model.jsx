...
const Model = ({ isMobile, ...props }) => {
    ...
    useFrame((state, delta) => {
        if (isMobile && modelRef.current) {
            ...
            // Convert 45 degrees to radians (45 * Math.PI / 180)
            const maxRotation = Math.PI / 4; // 45 degrees in radians
            // Use sine to oscillate between -45 and 45 degrees, with 180-degree offset
            modelRef.current.rotation.x = (Math.sin(0.005 * timeRef.current * Math.PI) * maxRotation) + Math.PI;
            // Keep the y rotation as is
            modelRef.current.rotation.y += delta * 0.03;
        }
    });
